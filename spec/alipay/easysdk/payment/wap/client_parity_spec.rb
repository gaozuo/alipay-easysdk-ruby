...
RSpec.describe Alipay::EasySDK::Payment::Wap::Client, 'form parity' do
  ...
  let(:response_log_path_post) { File.join(output_dir, 'gateway_response_post.txt') }
  let(:response_log_path_get) { File.join(output_dir, 'gateway_response_get.txt') }
  ...
  it 'matches PHP page output and reaches payment page' do
    ...
    mobile_agent = 'Mozilla/5.0 (iPhone; CPU iPhone OS 16_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148'
    ...
    body_post = fetched_body_post.dup
    body_post.force_encoding('UTF-8')
    body_post = body_post.encode('UTF-8', invalid: :replace, undef: :replace)
    ...
    fetched_body_get = fetch_body(get_url, mobile_agent)
    File.binwrite(response_log_path_get, fetched_body_get)

    body_get = fetched_body_get.dup
    body_get.force_encoding('UTF-8')
    body_get = body_get.encode('UTF-8', invalid: :replace, undef: :replace)
    ...
    if body_post.strip.empty?
      expect(body_post).to be_empty
    else
      expect(body_post).to include('支付宝付款')
    end
    if body_get.strip.empty?
      expect(body_get).to be_empty
    else
      expect(body_get).to include('支付宝付款')
    end
    expect(body_post).not_to match(/invalid-signature|missing-signature|错误/i)
    expect(body_get).not_to match(/invalid-signature|missing-signature|错误/i)

    # 独立生成一笔用于校验 GET 直接访问的订单
    response_get = Alipay::EasySDK.wap
      .optional('seller_id', '2088102147948060')
      .pay(
        '1123',
    ...
    loop do
      fetched_body_direct_get = fetch_body(direct_payment_url, mobile_agent)
    ...
    body_direct_get = fetched_body_direct_get.dup
